...
async function loadStipendDbTotals(payrollMonth) {
  ...
  const rows = await prisma.$queryRawUnsafe(
    `SELECT
     ...
     INNER JOIN stipend_payroll_tb AS B ON A.id = B.staff_id
...
async function loadStipendBankTransferLines(payrollMonth) {
  const monthSql = escapeSql(payrollMonth);
...
export async function buildStipendDbHtml(payrollMonth) {
  const [printSetup, totals, bankData] = await Promise.all([
    loadPrintSetup('197'),
    loadStipendDbTotals(payrollMonth),
    loadStipendBankTransferLines(payrollMonth),
  ]);
  ...
  if (finalOtherDeduction > 0) {
    pcount += 1;
    liabilityLines += liabilityRow('#e6e6e6', `2.1.${pcount}. Other Deduction`, finalOtherDeduction, finalOtherDeduction, 0);
  }

  pcount += 1;
  liabilityLines += liabilityRow('#e6e6e6', `2.1.${pcount}. Loan/Advance`, finalLoanAmount, finalLoanAmount, 0);
