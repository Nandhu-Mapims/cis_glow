...
async function loadCellAllocations(scope, day, period) {
  const rows = await prisma.$queryRawUnsafe(
    `SELECT subject_id, batch_no, staff_id FROM timetable_tb_new
     WHERE del=1 AND course_id='${escapeSql(String(scope.courseId))}'
       ...
       AND academic_type='${escapeSql(scope.academicType)}' AND t_day='${escapeSql(day)}' AND period='${escapeSql(String(period))}'
     ORDER BY id ASC`,
  );
...
export async function loadTtConfigV3(memberId, fields = {}, audit = {}) {
  ...
  if (selection && semester && loadGrid) {
    ...
    if (tt.first && tt.time[tt.first]) {
      ...
      for (let i = 1; i <= tt.max; i += 1) {
        ...
        headers.push({ from: pt.ftime, to: pt.ttime, colspan, periodOrder: i });
...
export async function saveTtConfigV3() {
  return { success: false, message: 'Use timetable cell editor to save allocations (v3)' };
}
