...
const PAGE = 'feedback_report.php';
...
export async function loadFeedbackReportUg(memberId, fields = {}, audit = {}) {
  const feedbackId = Number(fields.category) || 0;
  const reportType = ['overall', 'theory', 'practical', 'online'].includes(fields.report_type)
    ? fields.report_type
    : 'subject';
  ...
  if (generate && selectedFeedback) {
    if (reportType === 'subject' && selected && subjectIds.length) {
      ...
      html = await buildOverallFeedbackHtml(feedbackId, selectedFeedback.title, reportType, courseList);
      ...
      html = wrapReportPanel('<p class="text-muted mb-0">Select a course and at least one subject, or choose an Overall report type.</p>');
    }
  }
...
export async function saveFeedbackReportUg(memberId, fields = {}, audit = {}) {
  return loadFeedbackReportUg(memberId, fields, audit);
}
