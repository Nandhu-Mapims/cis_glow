...
export async function loadPayrollEmailerSetup(memberId, fields = {}, audit = {}) {
...
export async function savePayrollEmailerSetup(payload, memberId, audit = {}) {
  ...
  if (payload.action === 'delete') {
    ...
    return {
      ...
      ...(await loadPayrollEmailerSetup(memberId, { cronRef: payload.cronRef }, { ...audit, skipLog: true })),
    };
  }
  ...
  for (const row of rows) {
    ...
    if (!row.id) {
      await prisma.basic_cron_email.create({
        data: { cron_id: cronId, email_name: name, email_id: email, mobile_no: '', ...create },
      });
    } else {
      await prisma.basic_cron_email.update({
        where: { id: Number(row.id) },
        data: { email_name: name, email_id: email, del: 1, ...update },
      });
    }
  }
  ...
  return {
    ...
    ...(await loadPayrollEmailerSetup(memberId, { cronRef: String(cronId) }, { ...audit, skipLog: true })),
  };
}
