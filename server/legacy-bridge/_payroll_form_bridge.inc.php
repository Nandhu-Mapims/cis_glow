...
function payroll_form_bridge_run(string $legacyPhpFile): void
{
    ...
    if (preg_match_all('/<script>(.*?)<\/script>/s', $full, $scriptMatches)) {
        ...
        foreach ($scriptMatches[1] as $chunk) {
            ...
            if (stripos($chunk, 'multipleSelect') !== false
                ...
                || stripos($chunk, 'att_report_span') !== false
                || stripos($chunk, '$(document).ready') !== false
            ) {
                $chunks[] = $chunk;
            }
        }
    ...
    if ($html === '') {
        echo json_encode([
            'error' => 'Unable to parse payroll report HTML',
            'page' => $legacyPhpFile,
