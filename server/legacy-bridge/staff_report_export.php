...
$getParams = [
    'course_name' => $input['categoryId'] ?? '',
    'academic_year' => $input['academicYear'] ?? '',
    'academic_section' => $input['academicSection'] ?? '',
...
if ($format === 'xls') {
    ...
    $before = glob($excelDir . '/staff_profile_report_*.xlsx') ?: [];
    $beforeTimes = array_map('filemtime', $before);

    ob_start();
    include $legacyRoot . '/staff_profile_export_xls.php';
    ob_end_clean();

    $after = glob($excelDir . '/staff_profile_report_*.xlsx') ?: [];
    $newFile = null;
...
ob_start();
include $legacyRoot . '/staff_profile_export_more.php';
echo ob_get_clean();
