...
require __DIR__ . '/payroll_config_helper.php';
...
$screens = payroll_config_screen_map();
$screen = $input['screen'] ?? '';
if (!isset($screens[$screen])) {
    echo json_encode(['error' => 'Unknown payroll setup screen']);
    exit(1);
}
...
payroll_config_apply_post($fields);
...
$result = payroll_config_extract($full);
...
if (!$result['success'] && $screen === 'stipend-payroll-close' && ($fields['Submit'] ?? '') === 'Update') {
    $result['success'] = stripos($full, 'alert-danger') === false;
    if ($result['success'] && $result['message'] === '') {
        $result['message'] = 'Payroll status updated.';
    }
}
