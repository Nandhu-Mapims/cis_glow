...
require __DIR__ . '/exam_config_helper.php';
...
$screens = exam_config_screen_map();
$screen = $input['screen'] ?? '';
if (!isset($screens[$screen])) {
    echo json_encode(['error' => 'Unknown exam screen']);
    exit(1);
}
...
$result = exam_config_extract($full);
$result['success'] = $result['success'] || stripos($full, 'Your details are Updated') !== false
    ...
    || stripos($full, 'uploaded successfully') !== false
    || stripos($full, 'alert-success') !== false;
