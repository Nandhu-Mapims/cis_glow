...
export default function StudentAttachments({ studentId }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get(`/api/students/${studentId}/attachments`);
  ...
  const handleFile = async (index, file) => {
    ...
    try {
      ...
      const res = await api.post(`/api/students/${studentId}/attachments/upload`, {
        filename: file.name,
        dataBase64,
      });
      updateRow(index, {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await api.put(`/api/students/${studentId}/attachments`, {
        items: types.map((t) => ({
          attachId: t.attachId,
          recordId: t.recordId,
          attachNo: t.attachNo,
  ...
  return (
    <div className="card shadow-sm">
      ...
      <div className="card-body">
        ...
        <p className="text-muted small">Supported: jpg, png, gif, doc, docx, pdf</p>

        {types.length === 0 && (
          <div className="text-muted">No attachment types configured for this course.</div>
        )}
              ...
              {types.map((row, index) => (
                <tr key={row.attachId}>
                  <td>
                    ...
                    <div className="text-muted small">{row.shortName}</div>
                  </td>
                  <td>
                    <input
