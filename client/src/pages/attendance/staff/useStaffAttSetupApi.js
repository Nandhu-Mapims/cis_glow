...
export function useStaffAttSetupApi(screen) {
  ...
  const load = useCallback(async (fields = {}, query = {}) => {
    ...
    setError(null);
    try {
      const res = await api.post(`/api/attendance/staff/setup/${screen}/load`, { fields, query });
      if (res.data.error) {
  ...
  const save = useCallback(async (fields) => {
    ...
    setNotice(null);
    try {
      const res = await api.post(`/api/attendance/staff/setup/${screen}/save`, { fields });
...
export function useStaffAttScreenApi(screen) {
  const [data, setData] = useState(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState(null);
  const [notice, setNotice] = useState(null);
  ...
  const save = useCallback(async (fields) => {
    ...
    setNotice(null);
    try {
      const res = await api.post(`/api/attendance/staff/${screen}/save`, { fields });
      if (res.data.success === false) {
        setError(res.data.message || 'Save failed');
      } else if (res.data.message) {
        setNotice(res.data.message);
