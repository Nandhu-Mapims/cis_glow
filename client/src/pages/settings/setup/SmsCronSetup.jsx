...
function emptyContact() {
  return { key: `new-${Date.now()}`, name: '', mobile: '' };
}

export default function SmsCronSetup({ data, busy, onLoad, onSave }) {
  ...
  const handleSave = async (e) => {
    ...
    await onSave({
      ...
      cronStatus,
      cronDay,
      contacts: contacts.map(({ id, name, mobile }) => ({ id, name, mobile })),
    });
  };
    ...
    <>
      ...
      <div className="row g-3 mb-3">
        <div className="col-md-4">
          <label className="form-label">Cron job</label>
        ...
        <>
          <div className="row g-3 mb-3">
            ...
            <div className="col-md-2">
              ...
              <input className="form-control" value={cronDay} onChange={(e) => setCronDay(e.target.value)} />
            </div>
          </div>
            ...
            <div className="table-responsive">
              <table className="table table-bordered align-middle">
                <thead className="table-secondary">
                  <tr><th>Name</th><th>Mobile</th><th /></tr>
                </thead>
                <tbody>
                  {contacts.map((row, index) => (
                    <tr key={row.key}>
                      <td><input className="form-control" value={row.name} onChange={(e) => updateContact(index, { name: e.target.value })} /></td>
                      <td><input className="form-control" value={row.mobile} onChange={(e) => updateContact(index, { mobile: e.target.value })} /></td>
