...
export default function AddSetup({ data, busy, onLoad, onSave }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form, file ? [file] : []); }} className="row g-3">
      ...
      <div className="col-md-3"><label className="form-label">From Date</label><input type="date" className="form-control" value={form.fromDate || ''} onChange={(e) => set('fromDate', e.target.value)} /></div>
      <div className="col-md-3"><label className="form-label">To Date</label><input type="date" className="form-control" value={form.toDate || ''} onChange={(e) => set('toDate', e.target.value)} /></div>
