...
export default function AdmissionExamSetup() {
  const { data, busy, error, notice, load, save } = useAcademicSetupApi('admission-exam');
      ...
      <div className="mb-3 row g-2">
        ...
        <div className="col-sm-5">
          <select className="form-select" value={courseYearKey} onChange={(e) => onCourseYearChange(e.target.value)}>
            ...
            {(data?.courseYearOptions || []).map((opt) => (
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.key}>
                    ...
                    <td><label><input type="checkbox" checked={row.examInternal} onChange={(e) => updateRow(index, { examInternal: e.target.checked })} /> Yes</label></td>
                    <td><label><input type="checkbox" checked={row.examViva} onChange={(e) => updateRow(index, { examViva: e.target.checked })} /> Yes</label></td>
                    <td><label><input type="checkbox" checked={row.examExternal} onChange={(e) => updateRow(index, { examExternal: e.target.checked })} /> Yes</label></td>
                    <td><label><input type="checkbox" checked={row.markOption} onChange={(e) => updateRow(index, { markOption: e.target.checked })} /> Yes</label></td>
                    <td><label><input type="checkbox" checked={row.examStatus} onChange={(e) => updateRow(index, { examStatus: e.target.checked })} /> Yes</label></td>
                    <td>
                      {row.id ? (
