...
export default function ExamNodueSetup({ readOnly }) {
  const { data, busy, error, notice, load, save } = useExamSetupApi('exam-nodue');
  ...
  const noDueOptions = data?.noDueOptions || [];
  const selectedExam = (data?.examOptions || []).find((opt) => opt.value === examId);
    ...
    <ExamSetupShell notice={notice} error={error} busy={busy}>
      ...
      {selectedExam?.group ? (
        <div className="mb-3 row g-2">
          ...
          <div className="col-sm-8 text-muted small">{selectedExam.group}</div>
        </div>
      ) : null}
      ...
      {data?.infoMessage && !students.length ? (
