...
export default function SecurityDepositAddSetup({ data, save, busy }) {
    ...
    <form onSubmit={onSubmit} encType="multipart/form-data">
      {lastAdded?.createdBy && (
        <div className="text-end text-muted small mb-3">
          Last Added by <span className="text-danger">{lastAdded.createdBy}</span>
          {lastAdded.createdAt ? (
            <> on <span className="text-danger">{lastAdded.createdAt}</span></>
          ) : null}
        </div>
      )}
      <div className="row g-3">
        <div className="col-md-4">
          ...
          <input className="form-control" value={data?.depositNo || ''} readOnly />
        </div>
        ...
        <div className="col-md-4">
          ...
          <select name="advance_type" className="form-select" required defaultValue={data?.depositTypes?.[0]?.value || '370'}>
        ...
        <div className="col-md-4">
          ...
          <input name="advance_amount" className="form-control" required maxLength={11} inputMode="decimal" placeholder="Enter Amount" />
        </div>
        <div className="col-md-4">
        ...
        <div className="col-md-6">
          ...
          <input name="approved_by" className="form-control" required placeholder="Principal/Accounts Manager" />
        </div>
      </div>
      <div className="mt-3 d-flex gap-2">
        ...
        <button
          ...
          onClick={() => { setDetectionFrom(''); setNoOfMonths('1'); setHoldMonths([]); setSurety([]); }}
        >
          Reset
        </button>
      </div>
